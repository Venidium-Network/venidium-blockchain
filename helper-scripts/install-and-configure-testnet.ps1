...
Start-Process -WorkingDirectory (Get-Item .).FullName -FilePath "VenidiumSetup-1.0.0.exe" -ArgumentList "--silent" -Wait
...
if ( "$env:KEYS" -eq 'There are no saved private keys' )
{
    ...
    & "$env:localappdata\venidium-blockchain\app-1.0.0\resources\app.asar.unpacked\daemon\Venidium.exe" keys generate
}

& "$env:localappdata\venidium-blockchain\app-1.0.0\resources\app.asar.unpacked\daemon\Venidium.exe" configure --testnet true

Write-Output "Launching Venidium Blockchain"

Start-Process -WorkingDirectory "$env:localappdata\venidium-blockchain\app-1.0.0" -FilePath "$env:localappdata\venidium-blockchain\Venidium.exe"

...
from venidium.util.config import create_default_venidium_config, initial_config_file, load_config, save_config
...
class TestConfig:
    ...
    def test_create_config_new(self, tmpdir):
        """
        Test create_default_venidium_config() as in a first run scenario
        """
    ...
    def test_create_config_overwrite(self, tmpdir):
        """
        Test create_default_venidium_config() when overwriting an existing config.yaml
        """
        # When: using a clean directory
        root_path: Path = Path(tmpdir)
        config_file_path: Path = root_path / "config" / "config.yaml"
    ...
    def test_load_config(self, root_path_populated_with_config, default_config_dict):
        ...
        assert config["daemon_port"] == default_config_dict["daemon_port"] == 52700
        ...
        assert (
            ...
            == "e172a07f4e246178138ed5a27e58c34149134b1d560fe28b640a3a524655063d"
        )

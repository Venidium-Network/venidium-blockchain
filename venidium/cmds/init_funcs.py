...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if "xvm_target_address" not in config["farmer"]:
        print(f"Setting the xvm destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "xvm_target_address" not in config["pool"]:
        print(f"Setting the xvm destination address for coinbase reward to {all_targets[0]}")

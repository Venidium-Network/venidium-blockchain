...
from venidium.util.misc import format_bytes
from venidium.util.misc import format_minutes
from venidium.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total venidium farmed: {amounts['farmed_amount'] / units['venidium']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['venidium']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['venidium']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'venidium start wallet' and 'venidium wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'venidium wallet show'")
    else:
        print("Note: log into your key using 'venidium wallet show' to see rewards for each key")

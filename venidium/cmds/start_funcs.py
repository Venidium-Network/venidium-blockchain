...
from venidium.cmds.passphrase_funcs import get_current_passphrase
from venidium.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from venidium.util.keychain import KeyringMaxUnlockAttempts
from venidium.util.service_groups import services_for_groups
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    ...
    venidium = sys.argv[0]
    process = subprocess.Popen(f"{venidium} run_daemon --wait-for-unlock".split(), stdout=subprocess.PIPE)
    return process
...
async def async_start(root_path: Path, group: str, restart: bool) -> None:
    ...
    if daemon is None:
        print("Failed to create the venidium daemon")
        return None

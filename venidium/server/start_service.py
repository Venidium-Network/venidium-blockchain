...
from venidium.daemon.server import singleton, service_launch_lock_path
from venidium.server.ssl_context import venidium_ssl_ca_paths, private_ssl_ca_paths
...
class Service:
    def __init__(
        ...
        proctitle_name = f"venidium_{service_name}"
        ...
        venidium_ca_crt, venidium_ca_key = venidium_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = VenidiumServer(
            advertised_port,
            node,
            peer_api,
            node_type,
            ...
            (venidium_ca_crt, venidium_ca_key),
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for VenidiumServer to be closed")

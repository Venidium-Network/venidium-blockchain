...
from venidium.daemon.keychain_proxy import (
    KeychainProxy,
...
from venidium.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from venidium.protocols import wallet_protocol
from venidium.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from venidium.util.errors import Err, ValidationError
from venidium.util.ints import uint32, uint128
from venidium.util.keychain import Keychain
from venidium.util.lru_cache import LRUCache
from venidium.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from venidium.wallet.settings.settings_objects import BackupInitialized
...
from venidium.wallet.wallet_blockchain import ReceiveBlockResult
from venidium.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    server: Optional[VenidiumServer]
    log: logging.Logger
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'venidium keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSVenidiumConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSVenidiumConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSVenidiumConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSVenidiumConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSVenidiumConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSVenidiumConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSVenidiumConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:

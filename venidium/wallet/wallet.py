...
from venidium.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
from venidium.wallet.puzzles.puzzle_utils import (
...
from venidium.wallet.secret_key_store import SecretKeyStore
from venidium.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_venidium(self, venidium_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if venidium_amount < 0:
            utxos = await self.select_coins(abs(venidium_amount), exclude)
        else:
        ...
        venidium_amount = spend_value + venidium_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": venidium_amount}]

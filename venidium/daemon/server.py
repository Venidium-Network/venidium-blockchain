...
from venidium.cmds.init_funcs import check_keys, venidium_init
from venidium.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from venidium.daemon.keychain_server import KeychainServer, keychain_commands
from venidium.daemon.windows_signal import kill
from venidium.server.server import ssl_context_for_root, ssl_context_for_server
from venidium.ssl.create_ssl import get_mozilla_ca_crt
from venidium.util.venidium_logging import initialize_logging
from venidium.util.config import load_config
from venidium.util.json_util import dict_to_json_str
from venidium.util.keychain import (
    Keychain,
...
from venidium.util.service_groups import validate_service
from venidium.util.setproctitle import setproctitle
from venidium.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Venidium.")
    quit()
...
service_plotter = "venidium plots create"
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("venidium daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered VENIDIUM_ROOT
    os.environ["VENIDIUM_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with VENIDIUM_ROOT: {os.environ['VENIDIUM_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in venidium_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    venidium_init(root_path, should_check_keys=(not wait_for_unlock))

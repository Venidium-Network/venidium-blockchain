...
from venidium.util.validate_alert import create_alert_file, create_not_ready_alert_file, validate_alert_file

bitcoin_hash = None
venidium_dev_message = None
...
if status is True:
    ...
    print("")
    print("___________ VENIDIUM DEV MESSAGE ____________")
    while True:
        venidium_dev_message = input("Insert message from Venidium Dev: ")
        print(f"Venidium Dev message = {venidium_dev_message}")
        y_n = input("Does this look good (y/n): ").lower()
        if y_n == "y":
            break

    genesis_challenge_preimage = f"bitcoin_hash:{bitcoin_hash},venidium_dev_message:{venidium_dev_message}"

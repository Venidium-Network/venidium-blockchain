...
rm venidium-blockchain-gui/temp.json || true
( cd "$PWD/venidium-blockchain-gui" && git checkout HEAD -- package-lock.json ) || true
cd "$PWD" || true
...
npm uninstall -g @venidium-network/notarize-cli || true
